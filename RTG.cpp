...
RTG::RTG(Configuration const &configuration_) : helpers(*this)
{
	...
	{ // create the `window` and `surface` (where things get drawn):
		...
		window = glfwCreateWindow(configuration.surface_extent.width,
	...
	{	  // create the `device` (logical interface to the GPU) and the `queue`s to which we can submit commands:
		{ // look up queue indices:
			...
			for (auto const &queue_family : queue_families)
			{
				...
				// if it does graphics or compute, set the graphics queue family:
...
void RTG::recreate_swapchain()
{
	...
	if (!configuration.headless && configuration.debug)
	{
		...
		std::cout << string_VkPresentModeKHR(present_mode) << "\n";
		std::cout << string_VkSurfaceTransformFlagBitsKHR(capabilities.currentTransform) << "\n";
	}
}
