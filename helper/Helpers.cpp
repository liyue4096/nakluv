...
size_t Helpers::get_bytes_per_pixel(VkFormat format)
{
	switch (format)
	{
...
Helpers::AllocatedImage Helpers::create_image(VkExtent3D const &extent, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, MapFlag map)
{
	AllocatedImage image;
	image.format = format;
...
void Helpers::transfer_to_image(void *data, size_t size, AllocatedImage &target)
{
	...
	//[[maybe_unused]] size_t bytes_per_pixel = (target.format);
	[[maybe_unused]] size_t bytes_per_pixel = get_bytes_per_pixel(target.format);
