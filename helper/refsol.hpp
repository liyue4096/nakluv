...
namespace refsol
{
	...
	void Tutorial_destructor_workspace(
		...
		VkCommandPool command_pool,
		// frees:
		VkCommandBuffer *workspace_command_buffer) noexcept;
	...
	void Tutorial_destroy_framebuffers(
		...
		AllocatedImage_T *swapchain_depth_image,
		VkImageView *swapchain_depth_image_view,
		std::vector<VkFramebuffer> *swapchain_framebuffers) noexcept;
	...
	void Helpers_free(
		RTG_T const &rtg,
		Allocation_T *allocation) noexcept;
	...
	void Helpers_destroy_image(
		RTG_T const &rtg,
		// destroys:
		AllocatedImage_T &image) noexcept;
	...
	void RTG_destructor_per_workspace(
		VkDevice device,
		PerWorkspace_T *workspace) noexcept;
	...
	struct AllocationPtr
	{
		...
		explicit AllocationPtr(Allocation_T *allocation) : handle(&allocation->handle),
	...
	struct AllocatedBufferPtr
	{
		...
		explicit AllocatedBufferPtr(AllocatedBuffer_T *buffer) : handle(&buffer->handle),
																 size(&buffer->size),
	...
	struct AllocatedImagePtr
	{
		...
		explicit AllocatedImagePtr(AllocatedImage_T *image) : handle(&image->handle),
															  extent(&image->extent),
															  format(&image->format),
	...
	void Tutorial_destroy_framebuffers_impl(
		...
		std::vector<VkFramebuffer> *swapchain_framebuffers_) noexcept;
	template <typename RTG_T, typename AllocatedImage_T>
	void Tutorial_destroy_framebuffers(
		RTG_T const &rtg,
		// destroys:
	...
	void Helpers_free_impl(
		VkDevice rtg_device,
		AllocationPtr const &allocation) noexcept;
	template <typename RTG_T, typename Allocation_T>
	void Helpers_free(
		RTG_T const &rtg,
		Allocation_T *allocation) noexcept
	...
	void Helpers_destroy_buffer_impl(
		VkDevice rtg_device,
		AllocatedBufferPtr const &buffer_) noexcept;
	template <typename RTG_T, typename AllocatedBuffer_T>
	void Helpers_destroy_buffer(
		RTG_T const &rtg,
		// destroys:
	...
	void Helpers_destroy_image_impl(
		VkDevice rtg_device,
		AllocatedImagePtr const &image) noexcept;
	template <typename RTG_T, typename AllocatedImage_T>
	void Helpers_destroy_image(
		RTG_T const &rtg,
		// destroys:
	...
	void RTG_destructor_per_workspace_impl(
		...
		VkFence *workspace_available_,
		VkSemaphore *image_available_,
		VkSemaphore *image_done_) noexcept;
	template <typename PerWorkspace_T>
	void RTG_destructor_per_workspace(
		VkDevice device,
		PerWorkspace_T *workspace) noexcept
