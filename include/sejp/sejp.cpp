	...
	value parse(std::istream &from)
	{
		...
		auto read_exactly = [&read_char](std::string const &expect)
		{
			for (auto e : expect)
			{
				char c = read_char();
				if (c != e)
					throw std::runtime_error(std::string("parse error: expected '") + e + "', got '" + c + "'.");
			}
		};
		...
		auto read_number = [&from, &read_char](char first) -> double
		{
			...
			if (from.peek() == '.')
			{
				...
				char c = read_char();
				if (!('0' <= c && c <= '9'))
					throw std::runtime_error(std::string("parse error: wanted fraction digits, got '") + c + "'.");
				acc += c;
				digits();
			}
			...
			if (from.peek() == 'E' || from.peek() == 'e')
			{
				...
				char c = read_char();
				if (!('0' <= c && c <= '9'))
					throw std::runtime_error(std::string("parse error: wanted exponent digits, got '") + c + "'.");
				acc += c;
				digits();
			}
					...
					else if (c == 'u')
					{
						...
						for (uint32_t i = 0; i < 4; ++i)
						{
							...
							if ('0' <= c && c <= '9')
								...
								throw std::runtime_error(std::string("parse error: invalid character '") + c + "' in \\uNNNN escape.");
						}
						assert(value <= 0xffff);
		...
		while (root.index == -1U || !parents.empty())
		{
			...
			else if ((parents.back() & TypeBits) == Object)
			{
				...
				if (!map.empty())
				{
					...
					if (c != ',')
						throw std::runtime_error("parse error: expected ',' between object members.");
					skip_wsp();
					c = read_char();
				}
				if (c != '"')
					throw std::runtime_error("parse error: expecting '\"' at start of key.");
			...
			else if ((parents.back() & TypeBits) == Array)
			{
				...
				{
					if (c != ',')
						throw std::runtime_error(std::string("parse error: expected ',' between array entries; got '") + c + "'.");
					skip_wsp();
					c = read_char(); // actual first character of value
				}
