...
#include "../spv/shaders/shadow.vert.inl"
    ;
...
void Tutorial::ShadowPipeline::create(RTG &rtg, VkRenderPass render_pass, uint32_t subpass)
{
    ...
    { // the set0_ViewProjection layout holds uniform matrix of view projection:
        ...
        VK(vkCreateDescriptorSetLayout(rtg.device, &create_info, nullptr, &set0_ViewProjection));
    }
...
void Tutorial::ShadowPipeline::destroy(RTG &rtg)
{
    ...
    if (set0_ViewProjection != VK_NULL_HANDLE)
    {
        vkDestroyDescriptorSetLayout(rtg.device, set0_ViewProjection, nullptr);
        set0_ViewProjection = VK_NULL_HANDLE;
    }
